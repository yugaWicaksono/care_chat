...
- `const.py` — shared `MODEL`, `OLLAMA_OPTIONS`, `CLOUD_MODEL`. Lives at project root, same reasoning as `db.py`: both `repair_chat.py` and `product_chat.py` import it directly and independently, so it's app-wide config, not owned by either chat's domain package.
- `llm.py` — the only thing `repair_chat.py`/`product_chat.py` call to talk to a model: `chat(messages, tools=None) -> dict`, same normalized shape (`{"message": {"role", "content", "tool_calls"}}`) regardless of provider. Reads `LLM_PROVIDER` env var (`ollama` default, `anthropic` for the demo fallback) once at import. The Ollama path is a passthrough to `ollama.chat` (zero behavior change from before this module existed). The Anthropic path converts message/tool schema shapes (system message pulled out to a separate `system=` param, `role: tool` history entries turned into `tool_result` blocks keyed by synthesized `toolu_<idx>` ids, tool schemas flattened to `input_schema`) and converts the response back to the same normalized shape. Lives at project root for the same reason as `const.py`/`db.py` — both routers import it directly.
...
- **`qwen3.5:9b` was tried as a faster local alternative to `mistral-small` and rejected — it's a hybrid-reasoning model, and Ollama runs it with hidden "thinking" tokens on by default.** Measured 1174 thinking tokens to answer "Hi" (163s), and up to 296s on a real repair prompt, on this hardware (MacBook Air M3 base chip, 24GB — 100% GPU-resident, so this isn't a CPU-fallback problem). Passing `think=False` fixes the latency (~5s) but breaks tool-calling: tested against the real repair system prompt + tools, `think=False` made the model narrate contact-detail requests instead of calling `lookup_customer` — the exact narrating-bug described above. Capping `num_predict` doesn't help either — thinking can exceed the cap and get cut off before any real answer or tool call is emitted (verified: empty output, no tool call, `done_reason: "length"`). `qwen2.5:7b-instruct` was also tried (fast, 4-19s, no thinking-mode issue) but skipped `lookup_customer` in 2 of 3 trials where it should have fired — same reliability bar `mistral-small` was originally chosen to clear. Net: on this hardware, local options trade speed for tool-calling reliability one way or the other; `mistral-small` stays the default because correctness beats latency for a ticket-filing flow, and `LLM_PROVIDER=anthropic` (see `llm.py` above) is the lever for when latency actually matters (e.g. a live demo).
