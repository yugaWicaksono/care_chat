...
- `repair_chat.py` — `APIRouter`: `GET /` (serves `static/index.html`) + `POST /chat` (repair dispatch loop — `create_replacement_request`/`lookup_customer` handling, the `verified_customers` override and required-verification check). Owns its own `sessions` store.
...
- `repair/` — `prompts.py` (loads `repair/protocols.json`, builds SYSTEM_PROMPT — also imports `CATALOG` from `product` and embeds it as background-only reference so the repair bot can recognize a named model and its specs; repair steps still only ever come from `protocols.json`), `protocols.json` (catalog: `product -> issue -> {severity: minor|major, steps: [...]}`, lowercase keys — extend here to support new products, no code change needed; plus an optional sibling `models -> model_name -> issue -> {severity, steps}` for model-specific overrides, checked first by `lookup_severity` and the prompt when a specific catalog model is known, falling back to the generic `product` entry when no model-specific entry exists — a model with no `models` entry just uses the generic steps, no code change needed to add more), `__init__.py` re-exports `SYSTEM_PROMPT`. Named `repair/`, not `prompt/` — it's specifically the repair chat's domain package (system prompt, protocol catalog); `product/` has its own `prompts.py` for the product-advice chat, so a generic "prompt" name stopped reflecting which domain it belonged to.
- `ticket/` — `tickets.py` (TOOL_SCHEMA, TICKET_FIELDS, lookup_severity, insert_ticket, list_tickets, get_ticket, generate_ticket_id, create_replacement_request — both the Python function and the tool-call name the model sees are `create_replacement_request`; imports PROTOCOLS from `repair.prompts`, DATABASE_URL from `db`), `schema.sql` (`tickets` table, auto-run by the `db` docker-compose service on first start), `__init__.py` re-exports `TOOL_SCHEMA`, `create_replacement_request`, `list_tickets`, `get_ticket`. `product_model` (nullable) captures the specific catalog model name if the user named or confirmed one — separate from `product` (the generic type, e.g. "rolstoel"); left empty otherwise — the repair prompt is told to leave it empty rather than guess.
...
flowchart TB
  ...
  subgraph DOM["domain packages — plain functions, no FastAPI"]
    ...
    TICKET["ticket/<br/>create_replacement_request<br/>lookup_severity"]
...
- **Ticket creation requires a verified customer.** `create_replacement_request` only succeeds when its `client_number` matches an entry already cached in `session["verified_customers"]` (i.e. a prior `lookup_customer` call in this session found exactly one match). No match → the tool call is rejected server-side and the model is told to look the customer up first, regardless of how plausible the contact details it supplied look. This replaced an earlier, weaker guard (`fabricated_fields`) that only checked whether contact_name/contact_info/address appeared literally in the user's own messages — that stopped invented "John Doe" details but still let anyone file a ticket by typing real-looking contact info for someone not actually in the `customers` table. Deliberate tradeoff: a real person not yet in the `customers` table (walk-in, phone order) can no longer get a ticket through chat — they need to be added via SQL first (see the `customer/` **Lookup-only** rule) or handled outside the bot. False reject just makes the bot re-ask/redirect — that direction is fine.
...
- **Tickets in Postgres, not a file.** `ticket/tickets.py`'s `insert_ticket` does a plain `psycopg` insert, fresh connection per call, same reasoning as `find_customer` (no pool at this scale). No FK from `tickets.client_number` to `customers.client_number` — the app layer already requires a verified customer before calling `insert_ticket` (see below), a DB-level FK would be redundant enforcement of the same rule, not a stricter one.
...
- **`static/tickets.html` escapes all user-influenced fields** (`product`, `product_model`, `issue`, `contact_name`, `contact_info`, `address`, `notes`) before inserting into `innerHTML` — those values ultimately come from chat input relayed through the model, so treat them as untrusted on this page. `severity` is not escaped since it's server-resolved from a fixed vocabulary (`minor`/`major`/`unknown`), never free text.
